...
class EncoderBlock(nn.Module):
    def __init__(self, key_size, query_size, value_size, num_hiddens, normal_shape, ffn_num_input,
...
class TransformerEncoder(d2l.Encoder):
    def __init__(self, vocab_size, key_size, query_size, value_size, num_hiddens, norm_shape, ffn_num_input,
        ...
        for i in range(num_layers):
            self.blks.add_module("black" + str(i),
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, X, state):
        ...
        # 因此 `state[2][self.i]` 包含着直到当前时间步第 `i` 个块解码的输出表⽰
